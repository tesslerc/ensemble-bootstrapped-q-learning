...
from agents import rainbow, dqn
from env import Env
...
if args.agent == 'dqn' or args.agent == 'ddqn' or args.agent == 'ebql' or args.agent == 'ensm-dqn' or args.agent == 'maxmin-dqn':
    ...
    args.learn_start = 50000
    args.evaluation_interval = 1000000
    args.evaluation_episodes = 20
    args.priority_exponent = 0
    exploration_start = 1.
    exploration_end = .1
    exploration_decay = 1000000
...
def get_epsilon(timesteps):
    fraction = min(float(timesteps) / exploration_decay, 1.)
    return exploration_start + fraction * (exploration_end - exploration_start)
...
if args.evaluate:
    ...
    avg_reward, avg_Q = test(args, 0, agent, val_mem, metrics, results_dir, evaluate=True)  # Test
    ...
    for T in trange(1, args.T_max + 1):
        ...
        action = agent.act_e_greedy(state, get_epsilon(T))  # Choose an action greedily (with noisy weights)
